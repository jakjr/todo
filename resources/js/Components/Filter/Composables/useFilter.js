...
export function useFilter(url, method, data, defaults) {
    //const rememberKey = typeof args[0] === 'string' ? args[0] : null
    const rememberKey = null
    //const data = (typeof args[0] === 'string' ? args[1] : args[0]) || {}
    const restored = rememberKey ? Inertia.restore(rememberKey) : null
    //let defaults = cloneDeep(data)
    let clean = Object.keys(defaults).reduce((acc, curr) => ({...acc, [curr]: ''}), {})
    let cancelToken = null
    let recentlySuccessfulTimeoutId = null
    let transform = data => data

    // console.log(defaults, clean, data)
    ...
    let form = reactive({
        ...
        reset() {
            console.log(defaults, clean, data, this.data())
            Object.assign(
                this,
                defaults //era clean
            )
